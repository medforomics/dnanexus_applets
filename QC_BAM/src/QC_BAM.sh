...
main() {
    ...
    docker run -v ${PWD}:/data docker.io/bcantarel/alignment:v1 java -Xmx64g -jar $PICARD/picard.jar CollectAlignmentSummaryMetrics R=reference/genome.fa I=${pair_id}.ontarget.bam OUTPUT=${pair_id}.alignmentsummarymetrics.txt
    docker run -v ${PWD}:/data docker.io/bcantarel/alignment:v1 java -Xmx64g -XX:ParallelGCThreads=1 -jar $PICARD/picard.jar EstimateLibraryComplexity I=consensus.bam OUTPUT=${pair_id}.libcomplex.txt
    docker run -v ${PWD}:/data docker.io/bcantarel/alignment:v1 samtools view  -@ 1 -b -q 1 consensus.bam | bedtools coverage -sorted -hist -g reference/genomefile.txt -b stdin -a hemepanelV3.bed > ${pair_id}.mapqualcov.txt
    ...
    docker run -v ${PWD}:/data docker.io/bcantarel/alignment:v1 java -Xmx64g -jar $PICARD/picard.jar CollectInsertSizeMetrics INPUT=consensus.bam HISTOGRAM_FILE=${pair_id}.hist.ps REFERENCE_SEQUENCE=reference/genome.fa OUTPUT=${pair_id}.hist.txt
