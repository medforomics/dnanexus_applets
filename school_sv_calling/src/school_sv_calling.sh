...
main() {
    ...
    docker run -v ${PWD}:/data docker.io/goalconsortium/structuralvariant:1.0.9 bash /seqprg/process_scripts/alignment/indexbams.sh
	...
	then
	    ...
	    docker run -v ${PWD}:/data docker.io/goalconsortium/structuralvariant:1.0.9 bash /seqprg/process_scripts/variants/svcalling.sh -r dnaref -p $caseid -l dnaref/itd_genes.bed -c $targetbed -a pindel -g GRCh38.86 -f
	...
	then
            docker run -v ${PWD}:/data docker.io/goalconsortium/structuralvariant:1.0.9 bash /seqprg/process_scripts/variants/svcalling.sh -r dnaref -b ${caseid}.tumor.bam -p ${caseid} -a ${a} -g GRCh38.86 $normopt -f
	elif [[ "${a}" == "itdseek" ]]
	then
	    docker run -v ${PWD}:/data docker.io/goalconsortium/structuralvariant:1.0.9 bash /seqprg/process_scripts/variants/svcalling.sh -r dnaref -b ${caseid}.tumor.bam -p ${caseid} -a ${a} -l dnaref/itd_genes.bed -g GRCh38.86 -f
	elif [[ "${a}" == "cnvkit" ]]
	then
	    docker run -v ${PWD}:/data docker.io/goalconsortium/structuralvariant:1.0.9 bash /seqprg/process_scripts/variants/cnvkit.sh -r dnaref -b ${caseid}.tumor.bam -p ${caseid} -d panel
