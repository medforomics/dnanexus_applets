...
main() {
    ...
    if [[ -z "${glist}" ]]
    then
        docker run -v ${PWD}:/data docker.io/goalconsortium/geneabund:0.5.31 -s ${stranded} -g /data/gencode.gtf -p ${pair_id} -b ${pair_id}.bam -i /data/gene_info.human.txt
    else
        dx download "$glist" -o genelist.txt
        docker run -v ${PWD}:/data docker.io/goalconsortium/geneabund:0.5.31 -s ${stranded} -g /data/gencode.gtf -p ${pair_id} -b ${pair_id}.bam -f /data/genelist.txt -i /data/gene_info.human.txt
    fi
