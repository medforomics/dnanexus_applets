#!/bin/bash
# bamct_alignQC 0.5.9
# Generated by dx-app-wizard.

main() {

    dx download "$Aligned_BAM" -o ${pair_id}.bam
    dx download "$reference" -o dnaref.tar.gz
    dx download "$Align_Stats" -o ${pair_id}.alignerout.txt

    tar xvfz dnaref.tar.gz

    USER=$(dx whoami)

    if [[ ${bamct} == 1 ]]
    then
        docker run -v ${PWD}:/data docker.io/goalconsortium/vcfannot:0.5.9 samtools index ${pair_id}.bam
        docker run -v ${PWD}:/data docker.io/goalconsortium/vcfannot:0.5.9 bam-readcount -w 0 -q 0 -b 25 -f dnaref/genome.fa ${pair_id}.bam > ${pair_id}.bamreadcount.txt

        bamreadct=$(dx upload ${pair_id}.bamreadcount.txt --brief)

        dx-jobutil-add-output bamreadct "$bamreadct" --class=file
    fi

    docker run -v ${PWD}:/data docker.io/goalconsortium/vcfannot:0.5.9 bash /usr/local/bin/bamqc.sh -p ${pair_id} -b ${pair_id}.bam -n rna
    docker run -v ${PWD}:/data docker.io/goalconsortium/gatk:0.5.9 perl /usr/local/bin/sequenceqc_rnaseq.pl -r dnaref -u $USER *.flagstat.txt

    fastqczip=$(dx upload ${pair_id}_fastqc.zip --brief)
    fastqchtml=$(dx upload ${pair_id}_fastqc.html --brief)
    seqstats=$(dx upload ${pair_id}.sequence.stats.txt --brief)

    dx-jobutil-add-output fastqczip "$fastqczip" --class=file
    dx-jobutil-add-output fastqchtml "$fastqchtml" --class=file
    dx-jobutil-add-output seqstats "$seqstats" --class=file
}
