...
main() {
    ...
    for i in *.bam
    do 
	docker run -v ${PWD}:/data docker.io/goalconsortium/abra2:1.0.9 samtools index -@ $threads $i
    done
        
    docker run -v ${PWD}:/data docker.io/goalconsortium/abra2:1.0.9 java -Xmx16G -jar /usr/local/bin/abra2.jar $ioopt --ref dnaref/genome.fa --threads $threads $opt --tmpdir tmpdir --mbq 150 --mnf 5 --mer 0.05 > abra.log
