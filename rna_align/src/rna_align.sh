...
main() {
    ...
    mkdir rnaref
    docker run -v ${PWD}:/data docker.io/goalconsortium/rna_alignment:1.0.4 tar -I pigz -xvf rnaref.tar.gz --strip-components=1 -C rnaref
    ...
    docker run -v ${PWD}:/data docker.io/goalconsortium/rna_alignment:1.0.4 bash /seqprg/process_scripts/alignment/rnaseqalign.sh -a ${aligner} -p ${sampleid} -r rnaref -x ${sampleid}.R1.fastq.gz -y ${sampleid}.R2.fastq.gz $umiopt
