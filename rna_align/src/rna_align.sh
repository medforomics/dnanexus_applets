...
main() {
    ...
    docker run -v ${PWD}:/data docker.io/goalconsortium/ralign:0.5.40 bash /seqprg/school/process_scripts/alignment/rnaseqalign.sh -a ${aligner} -p ${sampleid} -r rnaref -x ${sampleid}.R1.fastq.gz -y ${sampleid}.R2.fastq.gz $umiopt
