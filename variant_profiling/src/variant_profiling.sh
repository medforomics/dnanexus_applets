...
main() {
    ...
    if [[ "${algo}" == "msisensor" ]]
    then
        docker run -v ${PWD}:/data docker.io/goalconsortium/vcfannot:0.5.26 bash /seqprg/genomeseer/process_scripts/variants/msisensor.sh -r dnaref -p ${pair_id} -b ${pair_id}.tumor.bam -c targetpanel.bed $normopt
	msiout=$(dx upload ${pair_id}.msi --brief)
	dx-jobutil-add-output msiout "$msiout" --class=file
	
    elif [[ "${algo}" == "checkmates" ]]
    then
        docker run -v ${PWD}:/data docker.io/goalconsortium/vcfannot:0.5.26 python /usr/local/bin/ncm.py -B -d ./ -bed dnaref/NGSCheckMate.bed -O ./ -N ${pair_id}
        docker run -v ${PWD}:/data docker.io/goalconsortium/vcfannot:0.5.26 perl /seqprg/genomeseer/scripts/sequenceqc_somatic.pl -i ${pair_id}_all.txt -o ${pair_id}.sequence.stats.txt
