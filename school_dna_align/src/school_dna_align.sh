...
main() {
    ...
    docker run -v ${PWD}:/data docker.io/goalconsortium/dna_alignment:1.0.7 bash /seqprg/process_scripts/alignment/dnaseqalign.sh -r humanref -p ${sampleid} -x ${sampleid}.R1.fastq.gz -y ${sampleid}.R2.fastq.gz $alignopt
	...
	docker run -v ${PWD}:/data docker.io/goalconsortium/dna_alignment:1.0.7 bash /seqprg/process_scripts/alignment/virusalign.sh -b ${sampleid}.bam -p ${sampleid} -r virusref -f
