...
main() {
    ...
    mkdir rnaref
    docker run -v ${PWD}:/data docker.io/goalconsortium/ralign:0.5.36 tar -I pigz -xvf rnaref.tar.gz --strip-components=1 -C rnaref
    ...
    docker run -v ${PWD}:/data docker.io/goalconsortium/ralign:0.5.36 bash /seqprg/school/process_scripts/alignment/rnaseqalign.sh -a ${aligner} -p ${pair_id} -r rnaref -x ${pair_id}.trim.R1.fastq.gz -y ${pair_id}.trim.R2.fastq.gz $umiopt
