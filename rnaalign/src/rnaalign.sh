...
main() {
    ...
    docker run -v ${PWD}:/data docker.io/goalconsortium/trim_galore:0.5.29 -f -p ${pair_id} seq.R1.fastq.gz seq.R2.fastq.gz
    docker run -v ${PWD}:/data docker.io/goalconsortium/ralign:0.5.29 bash /seqprg/school/process_scripts/alignment/rnaseqalign.sh -a ${aligner} -p ${pair_id} -r rnaref -x ${pair_id}.trim.R1.fastq.gz -y ${pair_id}.trim.R2.fastq.gz $umiopt
