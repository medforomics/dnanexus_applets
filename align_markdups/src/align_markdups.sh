...
main() {
    ...
    mkdir humanref
    docker run -v ${PWD}:/data docker.io/goalconsortium/alignment:0.5.29 tar -I pigz -xvf humanref.tar.gz --strip-components=1 -C humanref
    docker run -v ${PWD}:/data docker.io/goalconsortium/alignment:0.5.29 tar -I pigz -xvf panel.tar.gz
    ...
    docker run -v ${PWD}:/data docker.io/goalconsortium/alignment:0.5.29 bash /seqprg/school/process_scripts/alignment/dnaseqalign.sh -r humanref -p ${pair_id} -x ${pair_id}.trim.R1.fastq.gz -y ${pair_id}.trim.R2.fastq.gz $alignopt
    docker run -v ${PWD}:/data docker.io/goalconsortium/alignment:0.5.29 bash /seqprg/school/process_scripts/alignment/markdups.sh -a ${mdup} -b ${pair_id}.bam -p ${pair_id} -r humanref
    
    mv ${pair_id}.dedup.bam ${pair_id}.consensus.bam
    mv ${pair_id}.dedup.bam.bai ${pair_id}.consensus.bam.bai
    USER=$(dx whoami)

    docker run -v ${PWD}:/data docker.io/goalconsortium/vcfannot:0.5.29 bash /seqprg/school/process_scripts/alignment/bamqc.sh -c targetpanel.bed -n dna -r ./ -b ${pair_id}.bam -p ${pair_id} -u $USER
	...
	docker run -v ${PWD}:/data docker.io/goalconsortium/alignment:0.5.29 bash /seqprg/school/process_scripts/alignment/virusalign.sh -b ${pair_id}.bam -p ${pair_id} -r virusref -f
