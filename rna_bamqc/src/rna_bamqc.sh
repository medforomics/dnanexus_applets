...
main() {
    ...
    then
        ...
        docker run -v ${PWD}:/data docker.io/goalconsortium/vcfannot:0.5.28 samtools index ${pair_id}.bam
        docker run -v ${PWD}:/data docker.io/goalconsortium/vcfannot:0.5.28 bam-readcount -w 0 -q 0 -b 25 -f rnaref/genome.fa ${pair_id}.bam > ${pair_id}.bamreadcount.txt
    ...
    docker run -v ${PWD}:/data docker.io/goalconsortium/vcfannot:0.5.28 bash /seqprg/school/process_scripts/alignment/bamqc.sh -p ${pair_id} -b ${pair_id}.bam -n rna 
