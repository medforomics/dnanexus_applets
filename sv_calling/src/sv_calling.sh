...
main() {
    ...
    docker run -v ${PWD}:/data docker.io/goalconsortium/structuralvariant:0.5.26 bash /seqprg/genomeseer/process_scripts/alignment/indexbams.sh
	...
	then
            docker run -v ${PWD}:/data docker.io/goalconsortium/structuralvariant:0.5.26 bash /seqprg/genomeseer/process_scripts/variants/svcalling.sh -r dnaref -b ${pair_id}.tumor.bam -p ${pair_id} -a ${a} $normopt -f
            vcfsv+=" ${pair_id}.${a}.vcf.gz"
	    gffile+=" ${pair_id}.${a}.genefusion.txt"
	elif [[ "${a}" == "itdseek" ]]
	then
	docker run -v ${PWD}:/data docker.io/goalconsortium/structuralvariant:0.5.26 bash /seqprg/genomeseer/process_scripts/variants/svcalling.sh -r dnaref -b ${pair_id}.tumor.bam -p ${pair_id} -a ${a} -l dnaref/itd_genes.bed -f
	vcfsv+=" ${pair_id}.${a}_tandemdup.vcf.gz"
	elif [[ "${a}" == "cnvkit" ]]
	then
	    docker run -v ${PWD}:/data docker.io/goalconsortium/structuralvariant:0.5.26 bash /seqprg/genomeseer/process_scripts/variants/cnvkit.sh -r dnaref -b ${pair_id}.tumor.bam -p ${pair_id} -d panel
