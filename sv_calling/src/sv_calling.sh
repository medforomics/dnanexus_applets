...
main() {
    ...
    if [[ "${algo}" == "pindel" ]]
    then
	 docker run -v ${PWD}:/data docker.io/goalconsortium/structuralvariant:0.5.19 -r dnaref -p $pair_id -l dnaref/itd_genes.bed -a pindel -f
    ...
    then
        docker run -v ${PWD}:/data docker.io/goalconsortium/structuralvariant:0.5.19 -r dnaref -b ${pair_id}.tumor.bam -p ${pair_id} -a ${algo} $normopt -f
    ...
    then
	docker run -v ${PWD}:/data docker.io/goalconsortium/vcfannot:0.5.19 bash  /seqprg/genomeseer/process_scripts/variants/svcalling.sh -r dnaref -b ${pair_id}.tumor.bam -p ${pair_id} -a ${algo} -l dnaref/itd_genes.bed -f
