...
echo "docker run -v ${PWD}:/data docker.io/goalconsortium/structuralvariant:0.5.31 bash /seqprg/school/process_scripts/alignment/indexbams.sh"
...
do
    ...
    if [[ "${a}" == "pindel" ]]
    then
	echo "docker run -v ${PWD}:/data docker.io/goalconsortium/structuralvariant:0.5.31 bash /seqprg/school/process_scripts/variants/svcalling.sh -r dnaref -p $pair_id -l dnaref/itd_genes.bed -a pindel -f"
	vcfout+=" ${pair_id}.${a}.vcf.gz"
	vcfsv+=" ${pair_id}.${a}_tandemdup.vcf.gz"
	gffile+=" ${pair_id}.${a}.genefusion.txt"
    elif [[ "${a}" == "delly" ]] || [[ "${a}" == "svaba" ]]
    then
        echo "docker run -v ${PWD}:/data docker.io/goalconsortium/structuralvariant:0.5.31 bash /seqprg/school/process_scripts/variants/svcalling.sh -r dnaref -b ${pair_id}.tumor.bam -p ${pair_id} -a ${a} $normopt -f"
        vcfsv+=" ${pair_id}.${a}.vcf.gz"
	gffile+=" ${pair_id}.${a}.genefusion.txt"
    elif [[ "${a}" == "itdseek" ]]
    then
	echo "docker run -v ${PWD}:/data docker.io/goalconsortium/structuralvariant:0.5.31 bash /seqprg/school/process_scripts/variants/svcalling.sh -r dnaref -b ${pair_id}.tumor.bam -p ${pair_id} -a ${a} -l dnaref/itd_genes.bed -f"
	vcfsv+=" ${pair_id}.${a}_tandemdup.vcf.gz"
    elif [[ "${a}" == "cnvkit" ]]
    then
	echo "docker run -v ${PWD}:/data docker.io/goalconsortium/structuralvariant:0.5.31 bash /seqprg/school/process_scripts/variants/cnvkit.sh -r dnaref -b ${pair_id}.tumor.bam -p ${pair_id} -d panel"
