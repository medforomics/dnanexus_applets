...
main() {
    ...
    docker run -v ${PWD}:/data docker.io/goalconsortium/alignment:0.5.30 tar -I pigz -xvf panel.tar.gz
    ...
    docker run -v ${PWD}:/data docker.io/goalconsortium/alignment:0.5.30 bash /seqprg/school/process_scripts/alignment/dnaseqalign.sh -r humanref -p ${pair_id} -x ${pair_id}.trim.R1.fastq.gz -y ${pair_id}.trim.R2.fastq.gz $alignopt
	...
	docker run -v ${PWD}:/data docker.io/goalconsortium/alignment:0.5.30 bash /seqprg/school/process_scripts/alignment/virusalign.sh -b ${pair_id}.bam -p ${pair_id} -r virusref -f
