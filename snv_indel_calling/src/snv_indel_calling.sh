...
main() {
	...
	then
	    for i in *.bam
	    do
		docker run -v ${PWD}:/data docker.io/goalconsortium/variantcalling:0.5.26 bash /seqprg/genomeseer/process_scripts/variants/gatkrunner.sh -a gatkbam -b ${pair_id}.consensus.bam -r dnaref -p ${pair_id}
		mv $i $i.ori
	...
	if [[ "${a}" == "fb" ]] || [[ "${a}" == "platypus" ]]
	then
	    docker run -v ${PWD}:/data docker.io/goalconsortium/variantcalling:0.5.26 bash /seqprg/genomeseer/process_scripts/variants/germline_vc.sh -r dnaref -p ${pair_id} -a ${a} ${panelopt}
	    docker run -v ${PWD}:/data docker.io/goalconsortium/variantcalling:0.5.26 bash /seqprg/genomeseer/process_scripts/variants/uni_norm_annot.sh -g 'GRCh38.92' -r dnaref -p ${pair_id}.${a} -v ${pair_id}.${a}.vcf.gz
	    
	elif [[ "${a}" == "strelka2" ]] || [[ "${a}" == "mutect" ]] || [[ "${a}" == "shimmer" ]]
	then
	    if [[ -z "$Normal_BAM" ]]
	    then
		docker run -v ${PWD}:/data docker.io/goalconsortium/variantcalling:0.5.26 bash /seqprg/genomeseer/process_scripts/variants/germline_vc.sh -r dnaref -p ${pair_id} -a ${a} ${panelopt} ${ponopt}
	    else
		docker run -v ${PWD}:/data docker.io/goalconsortium/variantcalling:0.5.26 bash /seqprg/genomeseer/process_scripts/variants/somatic_vc.sh -r dnaref -p ${pair_id} -n ${pair_id}.normal.bam -t ${pair_id}.tumor.bam -a ${a} ${panelopt} ${ponopt}
	    fi
	    docker run -v ${PWD}:/data docker.io/goalconsortium/variantcalling:0.5.26 bash /seqprg/genomeseer/process_scripts/variants/uni_norm_annot.sh -g 'GRCh38.92' -r dnaref -p ${pair_id}.${a} -v ${pair_id}.${a}.vcf.gz
	else
