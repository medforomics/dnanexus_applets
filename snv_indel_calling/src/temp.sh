...
then
    if [[ -z "$Normal_BAM" ]]
    then
        echo "docker run -v ${PWD}:/data docker.io/goalconsortium/variantcalling:0.5.27 bash /seqprg/genomeseer/process_scripts/variants/germline_vc.sh -r dnaref -p ${pair_id} -a ${algo} -b $capturebed ${ponopt}"
    else
        echo "docker run -v ${PWD}:/data docker.io/goalconsortium/variantcalling:0.5.27 bash /seqprg/genomeseer/process_scripts/variants/somatic_vc.sh -r dnaref -p ${pair_id} -n ${pair_id}.normal.bam -t ${pair_id}.tumor.bam -a ${algo} -b $capturebed ${ponopt}"
    fi
	...
	if [[ "${a}" == "fb" ]] || [[ "${a}" == "platypus" ]]
	then
	    echo "docker run -v ${PWD}:/data docker.io/goalconsortium/variantcalling:0.5.27 bash /seqprg/genomeseer/process_scripts/variants/germline_vc.sh -r dnaref -p ${pair_id} -a ${a} -b $capturebed"
	...
	then
	    if [[ -z "$Normal_BAM" ]]
	    then
		echo "docker run -v ${PWD}:/data docker.io/goalconsortium/variantcalling:0.5.27 bash /seqprg/genomeseer/process_scripts/variants/germline_vc.sh -r dnaref -p ${pair_id} -a ${a} -b $capturebed ${ponopt}"
	    else
		echo "docker run -v ${PWD}:/data docker.io/goalconsortium/variantcalling:0.5.27 bash /seqprg/genomeseer/process_scripts/variants/somatic_vc.sh -r dnaref -p ${pair_id} -n ${pair_id}.normal.bam -t ${pair_id}.tumor.bam -a ${a} -b $capturebed ${ponopt}"
		fi
	    echo "docker run -v ${PWD}:/data docker.io/goalconsortium/variantcalling:0.5.27 bash /seqprg/genomeseer/process_scripts/variants/uni_norm_annot.sh -g 'GRCh38.92' -r dnaref -p ${pair_id}.${a} -v ${pair_id}.${a}.vcf.gz"
	else
